...
?>
<?php
include("../../../../include/link.php");$sql_s="select serial_number from y_exchange_goods order by serial_number desc limit 1";
...
$exchange_goods_odd_number=$_POST['exchange_goods_odd_number'];
$date=date("Y-m-d");
$time=date("G:i:s");

$sql_s="select exchange_goods_odd_number from y_exchange_goods where(exchange_goods_odd_number='$exchange_goods_odd_number')";
$result_s=mysql_query($sql_s);
$row_s=mysql_fetch_array($result_s,MYSQL_BOTH);
$exchange_goods_odd_number_s=$row_s['exchange_goods_odd_number'];

$if_exchange_goods=($exchange_goods_odd_number_s==$exchange_goods_odd_number)?'yes':'no';

$sql_s="select vip_card_number from y_performance_input where(serial_number='$exchange_goods_odd_number')";
$result_s=mysql_query($sql_s);
$row_s=mysql_fetch_array($result_s,MYSQL_BOTH);
...
$pay_money_way_p=$_POST['pay_money_way'];
$pay_money_money_p=$_POST['pay_money_money'];
$pay_money_way=implode(',',$pay_money_way_p);
$pay_money_money_c=count($pay_money_money_p);
$pay_money_money=0;
for($i=0;$i<$pay_money_money_c;$i++)
{
$pay_money_money=$pay_money_money+$pay_money_money_p[$i];
}

$value_pay=0;
for($i=0;$i<$pay_money_money_c;$i++)
{
if($pay_money_way_p[$i]=="value_card")
{
$value_pay=$pay_money_money_p[$i];
}
}
...
$value=$value_return-$value_pay;

$exchange_goods_integral=($pay_money_money-$return_money_money)/10;
...
$exchange_goods_drop_money_p=$_POST['exchange_goods_drop_money'];
$exchange_goods_actual_money_p=$_POST['exchange_goods_actual_money'];
$exchange_goods_drop_money_c=count($exchange_goods_drop_money_p);
$exchange_goods_actual_money_c=count($exchange_goods_actual_money_p);
$exchange_goods_drop_money=0;
$exchange_goods_actual_money=0;
for($i=0;$i<$exchange_goods_actual_money_c;$i++)
{
$exchange_goods_drop_money=$exchange_goods_drop_money+$exchange_goods_drop_money_p[$i];	
$exchange_goods_actual_money=$exchange_goods_actual_money+$exchange_goods_actual_money_p[$i];
}
$exchange_goods_piece_number=$exchange_goods_actual_money_c;
$exchange_goods_ratio=($exchange_goods_drop_money!=0)?$exchange_goods_actual_money/$exchange_goods_drop_money:0;



$sql_s="select service_person,service_shop from y_performance_input where (serial_number='$exchange_goods_odd_number')";
$result_s=mysql_query($sql_s);
$row_s=mysql_fetch_array($result_s,MYSQL_BOTH);
...
$weight_person=($service_person_c!=0)?($pay_money_money-$return_money_money)/$service_person_c:0;
$weight_shop=($service_shop_c!=0)?($pay_money_money-$return_money_money)/$service_shop_c:0;
...
$sql="INSERT INTO `y_exchange_goods` VALUES (
'$serial_number',
'$exchange_goods_odd_number',
'$date',
'$time',
'$if_exchange_goods',
'$vip_card_number',
'$exchange_goods_integral',
'$account_balance',
'$return_money_way',
'$return_money_money',
'$pay_money_way',
...
'$actual_money',
'$ratio',
'$exchange_goods_piece_number',
'$exchange_goods_drop_money',
'$exchange_goods_actual_money',
'$exchange_goods_ratio',
'$service_person_s',
'$service_shop_s',
'$weight_person',
'$weight_shop',
...
$odd_number=$exchange_goods_odd_number;
$date=$date;
$time=$time;
$odd_numbers=0;
$piece_numbers=0;
...
$time=$time;	
$serial_number=$serial_number_s+1;	
$sell_odd_number=$exchange_goods_odd_number;	
$vip_card_number=$vip_card_number;	

$vip_code=$vip_code_s;	
$vip_name=$vip_name_s;	
$integral=$exchange_goods_integral;	
$value=$value;
...
$slot_card_pay=0;
$value_card_pay=0;
$integral_exchange_pay=0;
$else_way_pay=0;
for($i=0;$i<$pay_money_money_c;$i++)
{
$cash_pay=($pay_money_way_p[$i]=='cash')?$pay_money_money_p[$i]:$cash_pay;
$cash_ticket_pay=($pay_money_way_p[$i]=='cash_ticket')?$pay_money_money_p[$i]:$cash_ticket_pay;
$slot_card_pay=($pay_money_way_p[$i]=='slot_card')?$pay_money_money_p[$i]:$slot_card_pay;
$value_card_pay=($pay_money_way_p[$i]=='value_card')?$pay_money_money_p[$i]:$value_card_pay;
$integral_exchange_pay=($pay_money_way_p[$i]=='integral_exchange')?$pay_money_money_p[$i]:$integral_exchange_pay;
$else_way_pay=($pay_money_way_p[$i]=='else_way')?$pay_money_money_p[$i]:$else_way_pay;
}


$cash=$cash_pay-$cash_return;
$cash_ticket=$cash_ticket_pay-$cash_ticket_return;
$slot_card=$slot_card_pay-$slot_card_return;
$value_card=$value_card_pay-$value_card_return;
$integral_exchange=$integral_exchange_pay-$integral_exchange_return;
$else_way=$else_way_pay-$else_way_return;
