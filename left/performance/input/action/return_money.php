...
?>
<?php
include("../../../../include/link.php");$sql_s="select serial_number from y_return_money order by serial_number desc limit 1";
...
$return_money_odd_number=$_POST['return_money_odd_number'];
$date=date("Y-m-d");
$time=date("G:i:s");

$sql_s="select exchange_goods_odd_number from y_exchange_goods where(exchange_goods_odd_number='$return_money_odd_number')";
$result_s=mysql_query($sql_s);
$row_s=mysql_fetch_array($result_s,MYSQL_BOTH);
$exchange_goods_odd_number_s=$row_s['exchange_goods_odd_number'];

$if_exchange_goods=($exchange_goods_odd_number_s==$return_money_odd_number)?'yes':'no';

$sql_s="select vip_card_number from y_performance_input where(serial_number='$return_money_odd_number')";
$result_s=mysql_query($sql_s);
$row_s=mysql_fetch_array($result_s,MYSQL_BOTH);
...
$return_money_integral=(-$return_money_money)/10;
...
$sql_s="select service_person,service_shop from y_performance_input where (serial_number='$return_money_odd_number')";
$result_s=mysql_query($sql_s);
$row_s=mysql_fetch_array($result_s,MYSQL_BOTH);
...
$sql="INSERT INTO `y_return_money` VALUES (
'$serial_number',
'$return_money_odd_number',
'$date',
'$time',
'$if_exchange_goods',
'$vip_card_number',
'$return_money_integral',
'$account_balance',
'$return_money_way',
'$return_money_money',
'$if_return_whole_odd',
...
$odd_number=$return_money_odd_number;
$date=$date;
$time=$time;
$odd_numbers=0;
$piece_numbers=0;
...
$time=$time;	
$serial_number=$serial_number_s+1;	
$sell_odd_number=$return_money_odd_number;	
$vip_card_number=$vip_card_number;	

$vip_code=$vip_code_s;	
$vip_name=$vip_name_s;	
$integral=$return_money_integral;	
$value=$value_return;	
